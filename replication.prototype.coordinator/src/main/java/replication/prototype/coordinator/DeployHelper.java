...
public class DeployHelper {
  ...
  private String filePath = "src" + s + "main" + s + "resources" + s + "Configuration.xml";
  ...
  private void generalInitializatiom() throws JAXBException {
    ...
    jaxbContext = JAXBContext.newInstance(ReplicationConfigurationType.class);
    Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
    this.config = (ReplicationConfigurationType) jaxbUnmarshaller.unmarshal(configFile);
  ...
  private void initializationForOnlineDeployment() throws Exception {
    ...
    /*
     * The ProfileCredentialsProvider will return your [New EU West (Ireland) Profile] credential
     * profile by reading from the credentials file located at (/Users/Tim/.aws/credentials).
     */
    AWSCredentials credentials = null;
    try {
      credentials = new ProfileCredentialsProvider("AEC").getCredentials();
    } catch (Exception e) {
      throw new AmazonClientException(
          "Cannot load the credentials from the credential profiles file. "
              + "Please make sure that your credentials file is at the correct "
              + "location (/Users/Tim/.aws/credentials), and is in valid format.", e);
    }
   ...
   * @throws IOException
   */
  private void deployNode(NodeType n, ReplicationConfigurationType config, int port)
      throws IOException {
    String startUrl = "http://" + n.getIpadress() + ":" + port + "/startServer/{thisNode}";

    RestTemplate restTemplate = new RestTemplate();
    // debug: check connection and url-parsing
    String runningUrl = "http://" + n.getIpadress() + ":" + port + "/isServerRunning";
    ...
    result = restTemplate.postForObject(startUrl, config, Boolean.class, n.getLabel());
  ...
  private void getInstanceAdressesForRegion(Region rg) {
    ...
    if (!this.adresses.containsKey(rg)) {
      ...
      // availabilityZonesResult.getAvailabilityZones().size() + " Availability Zones.");
  ...
  private Instance getNextIpAddress(Region currentRegion) throws Exception {
    if (this.adresses.containsKey(currentRegion) && !this.adresses.get(currentRegion).isEmpty()) {
